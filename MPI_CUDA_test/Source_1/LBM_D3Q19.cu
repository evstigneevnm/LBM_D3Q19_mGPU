...
void dispatch_info_data(const char *property_tree_file_name, communication_variables *COM)
{
    if(COM->myrank==MASTER)
    {
        ...
        for(int pr=0;pr<COM->totalrank;pr++)
        {
            ...
            real L0x=(MPI_sections=='X')?(real(globalLx*L_start/MPI_fraction)):0.0;
            real L0y=(MPI_sections=='Y')?(real(globalLy*L_start/MPI_fraction)):0.0;
            real L0z=(MPI_sections=='Z')?(real(globalLz*L_start/MPI_fraction)):0.0;            
...
int main(int argc, char *argv[])
{
    ...
    cudaStreamCreate(&COM.streams[1]);
    ...
    cudaStreamDestroy(COM.streams[1]);
